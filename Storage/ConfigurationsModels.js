...
export default class Configurations{
    ...
    async createTableConfiguration(){
        ...
        return new Promise((resolve, reject) => {
            db.transaction((tx) => {
                tx.executeSql(
                'CREATE TABLE IF NOT EXISTS Configuration (key TEXT UNIQUE PRIMARY KEY, state TEXT NOT NULL DEFAULT 0)', [], 
                (tx, results) => {
                    resolve(results) 
    ...
    async DeleteTableConfiguration(){
        const db = await this.initDB()
        return new Promise((resolve, reject) => {
            db.transaction((tx) => {
                tx.executeSql(
    ...
    async insertIntoConfiguration(data_to_insert){
